...
static func atomic_save(path: String, data: Variant) -> Error:
    ...
    if not DirAccess.dir_exists_absolute(base_dir):
        ...
        if err != OK:
            push_error(save_error_prefix + "Could not create directory %s (%s)" % [base_dir, error_string(err)])
            return err
    ...
    if file == null:
        ...
        push_error(save_error_prefix + "Could not open %s (%s)" % [temp_path, error_string(open_err)])
        return open_err
